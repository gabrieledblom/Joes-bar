...
/* In-memory-ersättning för Netlify Blobs */
const stores = new Map<string, Map<string, string>>();
vi.mock('@netlify/blobs', () => ({
  getStore: (opts: { name: string }) => {
    if (!stores.has(opts.name)) stores.set(opts.name, new Map());
    const data = stores.get(opts.name)!;
    return {
      ...
      list: async (o?: { prefix?: string }) => ({
        blobs: [...data.keys()].filter((k) => !o?.prefix || k.startsWith(o.prefix)).map((key) => ({ key })),
      }),
    };
  },
}));

import handler from './order.mts';
...
const skicka = (body: unknown, ip = '198.51.100.7') =>
  handler(
    new Request('http://localhost/.netlify/functions/order', {
      method: 'POST',
...
describe('order-funktionen', () => {
  ...
  it('loggar ordern till Blobs som säkerhetsnät', async () => {
    await skicka(giltigOrder());
    const logg = stores.get('orders')!;
    const post = JSON.parse(logg.get('order-2026-08-18-1')!);
  ...
  it('returnerar fel när restaurangens SMS inte går fram – ingen falsk bekräftelse', async () => {
    ...
    const post = JSON.parse(stores.get('orders')!.get('order-2026-08-18-1')!);
  ...
  it('rensar personuppgifter ur loggposter äldre än 30 dagar', async () => {
    const logg = stores.get('orders') ?? new Map();
    stores.set('orders', logg);
    logg.set(
      'order-2026-07-01-3',
      JSON.stringify({ ordernummer: '3', summa: 100, namn: 'Gammal', mobil: '+46700000009', kommentar: 'x' }),
    );
    await skicka(giltigOrder());
    const gammal = JSON.parse(logg.get('order-2026-07-01-3')!);
    expect(gammal.rensad).toBe(true);
    expect(gammal.mobil).toBeUndefined();
    expect(gammal.namn).toBeUndefined();
    expect(gammal.summa).toBe(100);
  });
});
